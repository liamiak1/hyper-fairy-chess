        ...
        {ranks.map((rank) =>
          files.map((file, fileIndex) => {
            ...
            return (
              <div
                key={posKey}
                className={`square ${isLightSquare(fileIndex, rank) ? 'light' : 'dark'} ${isSelected ? 'selected' : ''} ${isValidMove ? 'valid-move' : ''} ${isLastMoveSquare ? 'last-move' : ''} ${isValidPlacement ? 'valid-placement' : ''} ${isCapture ? 'capturable' : ''} ${isSpecialCaptureTarget ? 'special-capture-target' : ''}`}
              ...
              >
                {piece && pieceType && (
                  <div
                    className={`piece ${piece.owner} ${piece.isFrozen ? 'frozen' : ''} ${isCapture ? 'piece-capturable' : ''} ${isSpecialCaptureTarget ? 'piece-special-capture' : ''}`}
