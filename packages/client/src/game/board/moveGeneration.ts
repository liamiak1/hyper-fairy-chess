...
export function getAttackedSquares(
  ...
  for (const special of pType.movement.special) {
    switch (special) {
      ...
      case 'grasshopper':
        // Grasshopper attacks by hopping over a piece and landing beyond
        attacked.push(...generateGrasshopperMoves(board, piece));
        break;

      case 'cannon-move':
        // Cannon attacks by hopping over a screen piece to hit target
        attacked.push(...getCannonAttackSquares(board, piece));
        break;
    }
  }
...
/**
 * Get squares a Cannon attacks (capture squares only)
 * Cannon captures by hopping over exactly one piece (screen) to hit a target beyond
 */
function getCannonAttackSquares(board: BoardState, piece: PieceInstance): Position[] {
  if (!piece.position) return [];
  ...
  for (const dir of orthogonalDirs) {
    ...
    while (true) {
      ...
      if (!foundScreen) {
        // Looking for a screen piece
        if (isSquareEmpty(board, nextPos)) {
          currentPos = nextPos; // Keep sliding
        } else {
        ...
        // Already found screen - this square and beyond can be attacked
        if (isSquareEmpty(board, nextPos)) {
          currentPos = nextPos; // Keep sliding past empty
        } else {
          // Found a piece - this square is attacked (for check detection)
          attacked.push(nextPos);
          break; // Stop after finding a target
        }
      }
    }
  }
